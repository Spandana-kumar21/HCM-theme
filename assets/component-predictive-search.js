			...
			fetch(`${KROWN.settings.routes.predictive_search_url}?q=${encodeURIComponent(query)}&section_id=helper-predictive-search`)
				...
				.then(text=>{
					...
					if (KROWN.customer.isB2B !== 'true') {
						const searchItems = Array.from(parsedHTML.querySelectorAll('.search-item'));

						if (searchItems.length && searchItems.some(item => {
							return item.getAttribute('href')?.includes('rental')
						})) {
							const rentalItem = searchItems.find(item => item.getAttribute('href')?.includes('rental'));
							rentalItem?.remove()


							if (searchItems.length === 1) {
								parsedHTML.querySelector('.search-results').remove();
							}
						}
					}
					const results = parsedHTML.querySelector('#shopify-section-helper-predictive-search').innerHTML;
