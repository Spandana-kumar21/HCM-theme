...
class FacetFiltersForm extends HTMLElement {
  ...
  static renderPage(searchParams, event, updateURLHash = true) {
    let formattedSearchParams = searchParams;

    if (KROWN.customer.isB2B !== 'true') {
      const urlSearchParams = new URLSearchParams(formattedSearchParams);
    ...
    FacetFiltersForm.searchParamsPrev = formattedSearchParams;
    const sections = FacetFiltersForm.getSections();
    document.getElementById('main-collection-product-grid').classList.add('loading');
    sections.forEach((section) => {
      const url = `${window.location.pathname}?section_id=${section.section}&${formattedSearchParams}`;
      const filterDataUrl = element => element.url === url;
      FacetFiltersForm.filterData.some(filterDataUrl) ?
        FacetFiltersForm.renderSectionFromCache(filterDataUrl, event) :
        FacetFiltersForm.renderSectionFromFetch(url, event);
    });
    if (updateURLHash) FacetFiltersForm.updateURLHash(formattedSearchParams);
  }
